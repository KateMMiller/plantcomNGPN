...
#'
#' @description This function filters and joins FFI species cover data by park,
#' plot name, purpose, project, sample year, and other parameters. The data
#' returned are part of the target species protocol to aid early detection
#' efforts in parks. When no target species were detected during monitoring,
#' "no species" is recorded in the CommonName column, "NOSP" is the Symbol, and
#' ScientificName is blank.
#'
...
getCoverSpeciesComp <- function(park = 'all', plot_name = "all", project = "Park",
  ...
  covspp <- tryCatch(get("Cover_Species_Composition", envir = env),
                     error = function(e){
                       stop(paste0("Specified database does not contain Cover ",
  ...
  covspp_samp <- covspp[covspp$SampleEvent_GUID %in% sampev_guids,]
  ...
  keep_cols <- c("MacroPlot_Name", "Unit_Name", "MacroPlot_Purpose",
                 ...
                 "LifeCycle", "LifeForm_Name", "NotBiological",
                 "Species_Comment", "MacroPlot_GUID", "SampleEvent_GUID",
  ...
  sampcov_final <- covspp_samp2[order(covspp_samp2$MacroPlot_Name,
                                      covspp_samp2$SampleEvent_Date,
                                      covspp_samp2$Index),
                                final_names]

  if(nrow(sampcov_final) == 0){
