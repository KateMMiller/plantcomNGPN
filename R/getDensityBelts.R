...
#'
#' @description This function filters and joins FFI nested nested quadrat data
...
#' with c("_PCM_", "_FPCM_", "_LPCM_", and "_RCM_") in their name. If new
#' purposes are added in the future, they will need to be added to the bug
...
#' Current valid inputs:
#' \itemize{
#' \item{"NGPN_PCM":} {Default. Pulls in records with monitoring status base of "PlantCommunity".}
...
#'                         "FireOther_FuelReduction", "FPCM_Other_03",
#'                         "FireOther_3", "Fire_Other", "PCM_Fire", "Fire",
#'                         "Ext", "00Pre02")
#'
#' @param years Numeric. Filter on years. Accepted values start at 2011.
#' Default is 2011 to current year, which represents the time NGPN plant
#' community monitoring began using latest protocol and sample design.
#'
...
getDensityBelts <- function(park = 'all', plot_name = "all", project = "Park",
  ...
  densb <- tryCatch(get("Density_Belts_metric", envir = env),
                     error = function(e){
                       stop(paste0("Specified database does not contain Density",
                                   " Belts metric data."))})
  ...
  densb_samp <- densb[densb$SampleEvent_GUID %in% sampev_guids,]
  ...
  keep_cols <- c("MacroPlot_Name", "Unit_Name", "MacroPlot_Purpose",
                 ...
                 "SlopeTransect", "SampleEvent_Date", "year", "month",
                 "doy", "DefaultMonitoringStatus", #"MonitoringStatus_Name",
                 "Visited", "NumTran", "TranLen", "TranWid", "Area",
                 "Index", "Transect", "Subbelt", "SubFrac", "Status",
                 #"SizeCl", "AgeCl", "Height", blank in NGPN
                 "Count", "Comment", "UV1Desc", "UV2Desc", "SaComment",
                 "Symbol", "ITIS_TSN", "ScientificName", "CommonName",
                 "Nativity", "Invasive", "Cultural", "Concern", "LifeCycle",
                 "LifeForm_Name", "NotBiological", "UV1", "UV2", "UV3",
                 "MacroPlot_GUID", "SampleEvent_GUID", "RegistrationUnit_GUID",
                 "Spp_GUID")
  ...
  densb_final <- distinct(densb_samp2[order(densb_samp2$MacroPlot_Name,
                                            densb_samp2$SampleEvent_Date,
                                  densb_samp2$Transect,
                                  densb_samp2$Subbelt,
                                  densb_samp2$SubFrac,
                                  densb_samp2$ScientificName),
                                final_names])

  if(nrow(densb_final) == 0){
