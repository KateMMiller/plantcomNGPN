...
getTrees <- function(park = 'all', plot_name = "all", project = "Park", purpose = "NGPN_PCM",
  ...
  trees1 <- trees[trees$MonitoringStatus_Base %in% mon_status,]

  # Filter on sample events
  sampev_guids <- unique(sampev$SampleEvent_GUID)
  trees_samp <- trees1[trees1$SampleEvent_GUID %in% sampev_guids,]
