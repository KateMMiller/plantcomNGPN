...
#' }
#'
#' @return Returns a data frame of disturbance history data
#'
#' @export

getDisturbanceHistory <- function(park = 'all', plot_name = "all",
                                  project = "Park", purpose = "NGPN_PCM",
                                  mon_status = "NGPN_PCM",
  ...
  disturb <- tryCatch(get("Disturbance_History", envir = env),
                      error = function(e){
                        stop(paste0("Specified database does not contain ",
  ...
  dist_samp <- disturb[disturb$SampleEvent_GUID %in% sampev_guids,]
  ...
  keep_cols <- c("MacroPlot_Name", "Unit_Name", "MacroPlot_Purpose",
                 ...
                 "SampleEvent_GUID", "RegistrationUnit_GUID")
