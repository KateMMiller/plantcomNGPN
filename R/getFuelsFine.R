...
#' @title getFuelsFine
#'
#' @description This function filters and joins FFI Surface Fuels Fine data by
#' park, plot name, purpose, project, sample year, and other parameters.
#'
...
#' Community Monitoring plots (e.g.,macroplots with "_PCM_" will be included.
#' Can select multiple plots. If a plot name is specified that does not occur
#' in the imported data, function will error out with a list of unmatched plot
#' names.
#'
...
#'
#' @param purpose Quoted string to return plots with a particular purpose,
#' which typically refers to a characteristic of the plot's sample design in
#' NGPN (e.g., Panel1). Note that purpose is not standard across parks. This
#' function standardizes some purposes (eg "FX" and "Fire Effects" are both
#' called "FX monitoring"). The following purposes that can be specified are
#' below. By default, "NGPN_PCM" plots are selected, which includes all plots
#' with "_PCM_" in their name. If new purposes are added in the future, they
#' will need to be added to the bug handling code in the function.
#' Valid inputs:
#' \itemize{
#' \item{"all":} {All plots in imported FFI database}
#' \item{"NGPN_PCM":} {Default. NGPN Plant Community Monitoring Plots with "_PCM_" in their name, and all of the purposes defined below.}
...
#' MonitoringStatus$MonitoringStatus_Name types. Default is "NGPN_PCM", which
#' will pull in sample events coded a NGPN Plant Community Monitoring (see
#' description for NGPN_PCM below).
#' Current valid inputs:
#' \itemize{
...
#'                         "01yr01", "01yr02", "01yr10", "FireOther_1", "Dual",
#'                         "FPCM_Other_01", "FPCM_Other_02", "Other",
...
#' table(sffine_thro$Unit_Name, sffine_thro$ProjectUnit_Name, useNA = 'always')
#' head(sffine_thro)
#'
#' # get surface fine fuels ForestStructure monitoring status
#' sffine_for <- getFuelsFine(mon_status = "ForestStructure")
#' table(sffine_for$Unit_Name, sffine_for$MonitoringStatus_Base, useNA = 'always')
#'
#' }
#'
#' @return Returns a data frame of Fine surface fuels data
#'
#' @export

getFuelsFine <- function(park = 'all', plot_name = "all", project = "Park",
  ...
  sampev <- getSampleEvent(park = park,
                           plot_name = plot_name,
                           project = project,
                           purpose = purpose,
  ...
  sffine <- tryCatch(get("Surface_Fuels_Fine", envir = env),
                     error = function(e){stop(
                       paste0("Specified database does not contain Surface ",
                              "Fuels Fine data."))
                       })
  ...
  sffine_samp <- sffine[sffine$SampleEvent_GUID %in% sampev_guids,]
  ...
  keep_cols <- c("MacroPlot_Name", "Unit_Name", "MacroPlot_Purpose",
                 ...
                 "Visited",
                 "NumTran", "OneHrTranLen", "TenHrTranLen", "HunHrTranLen",
                 "Index", "Transect", "Azimuth_Fuels", "Slope", "OneHr",
                 "TenHr", "HunHr", "FWDFuConSt", "Comment", "UV1Desc", #"UV2Desc",
                 "SaComment", "MacroPlot_GUID", "SampleEvent_GUID",
                 "RegistrationUnit_GUID")

  final_names <- if(output == "short"){
    keep_cols
  } else {c(keep_cols, sort(setdiff(names(sffine_samp2), keep_cols)))}

  sffine_final <- distinct(sffine_samp2[order(sffine_samp2$MacroPlot_Name,
                                              sffine_samp2$SampleEvent_Date,
                                              sffine_samp2$Transect),
                                        final_names])

  if(nrow(sffine_final) == 0){
    warning("Specified arguments returned an empty dataframe.")
    }
