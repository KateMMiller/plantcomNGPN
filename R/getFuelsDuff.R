...
#' @title getFuelsDuff
#'
#' @description This function filters and joins FFI Surface Fuels Duff data by
#' park, plot name, purpose, project,
...
#' Community Monitoring plots (e.g., macroplots with "_PCM_" in their names)
#' will be included. Can select multiple plots. If a plot name is specified
#' that does not occur in the imported data, function will error out with a
#' list of unmatched plot names.
#'
...
#' (e.g., Panel1). Note that purpose is not standard across parks. This function
#' standardizes some purposes (eg "FX" and "Fire Effects" are both called "FX
#' monitoring"). The following purposes that can be specified are below. By
#' default, "NGPN_PCM" plots are selected, which includes all plots with "_PCM_"
#' in their name. If new purposes are added in the future, they will need to be
#' added to the bug handling code in the function.
#' Valid inputs:
#' \itemize{
...
#' DisturbanceHistory database tables.
#'
#' @examples
#' \dontrun{
#'
...
#' table(sfduff_thro$Unit_Name, sfduff_thro$ProjectUnit_Name, useNA = 'always')
#' head(sfduff_thro)
#'
#' # get surface duff fuels ForestStructure monitoring status
#' sfduff_for <- getFuelsDuff(mon_status = "ForestStructure")
#' table(sfduff_for$Unit_Name, sfduff_for$MonitoringStatus_Base, useNA = 'always')
#'
#' }
#'
#' @return Returns a data frame of Duff surface fuels data
#'
#' @export

getFuelsDuff <- function(park = 'all', plot_name = "all", project = "Park",
  ...
  ## get surface fuels duff
  sfduff <- tryCatch(get("Surface_Fuels_Duff", envir = env),
                     error = function(e){
                       stop(paste0("Specified database does not contain Surface ",
                                   "Fuels Duff data."))})
  ...
  sfduff_samp <- sfduff[sfduff$SampleEvent_GUID %in% sampev_guids,]
  ...
  keep_cols <- c("MacroPlot_Name", "Unit_Name", "MacroPlot_Purpose",
                 ...
                 "NumTran", "Index", "Transect", "SampLoc", "OffSet", "LittDep",
                 "DuffDep", "FuelbedDep", "DLFuConSt", "Comment", "UV1Desc", #"UV2Desc",
  ...
  sfduff_final <- distinct(sfduff_samp2[order(sfduff_samp2$MacroPlot_Name,
                                              sfduff_samp2$SampleEvent_Date,
                                              sfduff_samp2$Transect),
                                        final_names])

  if(nrow(sfduff_final) == 0){
