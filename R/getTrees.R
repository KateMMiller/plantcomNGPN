...
#' @title getTrees
#'
#' @description This function filters and joins FFI tree data by park, plot
#' name, purpose, project, sample year, and other parameters.
#'
...
#' table(trees_deto$Unit_Name, trees_deto$year, useNA = 'always')
#' head(trees_deto)
#'
#' # get tree data for ForestStructure monitoring status
#' trees_for <- getTrees(mon_status = "ForestStructure")
#' table(trees_for$Unit_Name, trees_for$MonitoringStatus_Base, useNA = 'always')
#'
#' }
#'
#' @return Returns a data frame of tree data
#'
#' @export

getTrees <- function(park = 'all', plot_name = "all", project = "Park",
  ...
  trees <- tryCatch(get("Trees_metric", envir = env),
                     error = function(e){
                       stop(paste0("Specified database does not contain Tree ",
                                   "metric data."))})
  ...
  trees_samp <- trees[trees$SampleEvent_GUID %in% sampev_guids,]

  # Drop records where not visited
  trees_samp2 <- trees_samp[trees_samp$Visited == TRUE,]
  ...
  keep_cols <- c("MacroPlot_Name", "Unit_Name", "MacroPlot_Purpose",
                 ...
                 "MacroPlotSize", "SnagPlotSize", "BrkPntDia", "QTR", "SubFrac",
                 "TagNo", "Symbol", "ITIS_TSN", "ScientificName", "CommonName",
                 "Status", "DBH", "CrwnCl", "LiCrBHt", "CrwnRad","DRC",
                 "Comment", "UV1", "UV2", "UV3","Nativity", "Invasive",
                 "Cultural", "Concern", "LifeCycle", "LifeForm_Name",
                 "NotBiological", "MacroPlot_GUID", "SampleEvent_GUID",
  ...
  trees_final <- distinct(trees_samp2[order(trees_samp2$MacroPlot_Name,
                                            trees_samp2$SampleEvent_Date,
                                   trees_samp2$QTR,
                                   trees_samp2$TagNo),
                            final_names])

  if(nrow(trees_final) == 0){
