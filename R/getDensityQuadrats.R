...
#'
#' @title getDensityQuadrats
#'
#' @description This function filters and joins FFI density quadrat data used
#' for seedlings and tall shrubs in NGPN by park, plot name, purpose, project,
...
#'                         "FPCM_Other_01", "FPCM_Other_02", "Other", "PCM_Other",
#'                         "FireOther_2", "FPCM_Other", "FireOther",
...
#' DensityQuadrats database tables.
#'
#' @examples
#' \dontrun{
#'
...
#' }
#'
#' @return Returns a data frame of seedling and tall shrub data
#'
#' @export

getDensityQuadrats <- function(park = 'all', plot_name = "all", project = "Park",
  ...
  keep_cols <- c("MacroPlot_Name", "Unit_Name", "MacroPlot_Purpose",
                 ...
                 "NumTran", "NumQuadTran", "QuadLen", "QuadWid", "Area",
                 "Index", "Transect", "Quadrat", "Status", "SizeCl", "AgeCl",
                 "Count", "SubFrac", "Comment", "UV1", #"Height", "UV2", "UV3",
                 "UV1Desc", #"UV2Desc",
                 "SaComment", "Symbol", "ITIS_TSN", "ScientificName",
  ...
  densq_final <- distinct(densq_samp2[order(densq_samp2$MacroPlot_Name,
                                            densq_samp2$SampleEvent_Date,
                                   densq_samp2$Transect,
                                   densq_samp2$Quadrat,
                                   densq_samp2$ScientificName),
                                final_names])

  if(nrow(densq_final) == 0){
