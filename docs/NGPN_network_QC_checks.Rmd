...
# macro_samp2 <- left_join(macro_samp, mm_monstat, 
#                          by = c("SampleEvent_GUID" = "MM_SampleEvent_GUID", "datasource"))
# 
# mac_samp3 <- left_join(macro_samp2, monstat,
...
# mac_samp4 <- mac_samp3 |> filter(year >= 2011) |>
#   mutate(park = substr(MacroPlot_Name, 1, 4)) |> 
#   group_by(park, MacroPlot_Name, MonitoringStatus_Base, year) |> 
...
#   arrange(plottype, plotnum, MonitoringStatus_Base) |> select(-plotnum, -plottype)
...
kable(df, format = 'html', align = 'c',
      caption = paste0('Matrix of NGPN PCM plots for **', parkcode, '** by monitoring status and year sampled.',
                       ...
                       "Sampling schedule does not exactly follow the 2 on 3 off, so Monitoring Status is not",
...
macro_purp <- macro_plots |> select(MacroPlot_Name, MacroPlot_Purpose) |> 
  #unique() |> 
  mutate(pres = 1, 
